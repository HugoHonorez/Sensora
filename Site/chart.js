const createChart = (ctx, datasets, yLabel) => new Chart(ctx, {
  ...
  const lightChart = createChart(document.getElementById("lightChart").getContext("2d"), [
    { label: "Luminosité (lux)", data: [], borderColor: 'goldenrod', backgroundColor: 'rgba(255,206,86,0.2)', tension: 0.3, pointRadius: 1 }
  ], "Luminosité (lux)");
  
  function clearCharts() {
    [tempChart, humidityChart, pressureChart, lightChart].forEach(chart =>
  ...
  function downloadCSV() {
    ...
    const headers = ['Time', 'Temperature (°C)', 'HeatIndex (°C)', 'Humidity (%)', 'Pressure (hPa)', 'Light'];
